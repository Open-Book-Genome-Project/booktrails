...
class DependencyBookGraph(core.Base):
    ...
    @classmethod
    def get_trailmap(cls, topic=None, depth=5):
        ...
        yield the most endorsed directed sequence of interdependent books for a topic.
        """
        trailmap = []  # the trailmap matrix to be built
        olids = set()
        seed = None  # the book seed (olid) for this level
        for d in range(depth):           
            ...
            trailmap.append(row)
            # Choose the most endorsed seed as next round's prereq
            seed = row[0]["book_olid"]

        return {"trailmap": trailmap, 'book_data': get_many(olids)}
